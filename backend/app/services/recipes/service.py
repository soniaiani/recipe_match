...
def _fetch_recipe(recipe_id: int) -> dict[str, Any]:
    admin = get_supabase_admin()
...
def get_ingredient_suggestion_response(q: str = "", limit: int = 20) -> ApiResponse[IngredientSuggestionsResponse]:
...
def get_recipe_detail_response(recipe_id: int) -> ApiResponse[RecipeDetail]:
...
def get_shopping_list_response(recipe_id: int) -> ApiResponse[ShoppingListResponse]:
    data = _fetch_recipe(recipe_id)
...
def get_similar_recipes_response(recipe_id: int, limit: int = _SIMILAR_LIMIT) -> ApiResponse[SimilarRecipesResponse]:
    admin = get_supabase_admin()
    safe_limit = max(1, min(limit, 50))
