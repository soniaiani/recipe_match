...
import {
  Dimensions, Pressable, ScrollView, StyleSheet, View,
} from 'react-native';
import Svg, {
...
const SATELLITE_NODE_RADIUS = 44;
const HIT_SIZE = 104;
...
function labelLines(label: string, maxChars = 10, maxLines = 3): string[] {
  ...
  if (lines.length > maxLines) {
    trimmed[maxLines - 1] = `${trimmed[maxLines - 1].slice(0, Math.max(maxChars - 1, 1))}...`;
  }
  return trimmed.map(line => (line.length > maxChars ? `${line.slice(0, Math.max(maxChars - 1, 1))}...` : line));
}
...
export function GraphCanvas({
  center, suggestions, previousNodes, relaxed, onNodePress,
}: GraphCanvasProps) {
  ...
  const canvasHeight = TOP_PADDING
    + Math.max(chainNodes.length, 1) * NODE_VERTICAL_SPACING
    + SATELLITES_AREA_HEIGHT
    + BOTTOM_PADDING;
  const centerX = canvasWidth / 2;
  const currentY = TOP_PADDING + Math.max(chainNodes.length - 1, 0) * NODE_VERTICAL_SPACING;
  const satelliteRadius = Math.min(
    SATELLITE_RADIUS,
    (canvasWidth / 2) - SATELLITE_NODE_RADIUS - HORIZONTAL_MARGIN,
  );
    ...
    <View style={styles.viewport}>
      ...
      >
        <View style={[styles.canvas, { height: canvasHeight }]}>
            ...
            {chainNodes.map((node, index) => {
              ...
              return (
                <React.Fragment key={`${node}-${index}`}>
                  ...
                  />
                  {labelLines(node, isCurrent ? 15 : 12).map((line, lineIndex, lines) => {
                    ...
                    return (
                      <SvgText
                        key={`${node}-${line}`}
                        x={centerX}
                        y={y + (lineIndex - (lines.length - 1) / 2) * lineHeight + fontSize / 3}
          ...
          <Animated.View pointerEvents="none" style={[StyleSheet.absoluteFill, satelliteFadeStyle]}>
            <Svg width={canvasWidth} height={canvasHeight}>
              {suggestions.map((item, index) => {
                const angle = satelliteAngle(index, suggestions.length) * (Math.PI / 180);
                ...
                return (
                  <React.Fragment key={`satellite-${item.ingredient}`}>
                    <Line
                      ...
                      strokeDasharray="4 3"
                    />
                    <Circle cx={x} cy={y} r={SATELLITE_NODE_RADIUS} fill={Colors.accent} />
                    {labelLines(item.ingredient, 12).map((line, lineIndex, lines) => {
          ...
          {selectedSatellite ? suggestions.map((item, index) => {
            ...
            return (
              <React.Fragment key={`selected-${item.ingredient}`}>
                ...
                <Animated.View
                  pointerEvents="none"
                  style={[styles.selectedNode, { left: x - HIT_SIZE / 2, top: y - HIT_SIZE / 2 }, pulseStyle]}
                >
                  <Svg width={HIT_SIZE} height={HIT_SIZE}>
                    <Circle cx={HIT_SIZE / 2} cy={HIT_SIZE / 2} r={SATELLITE_NODE_RADIUS} fill={Colors.accent} />
                    {labelLines(item.ingredient, 12).map((line, lineIndex, lines) => {
                      const fontSize = nodeFontSize(item.ingredient, 11);
                      const lineHeight = fontSize + 2;
                      return (
                        <SvgText
                          ...
                          x={HIT_SIZE / 2}
                          y={HIT_SIZE / 2 + (lineIndex - (lines.length - 1) / 2) * lineHeight + fontSize / 3}
                          fill={Colors.textInverse}
                          fontSize={fontSize}
                          fontWeight="700"
                          textAnchor="middle"
          ...
          {suggestions.map((item, index) => {
            const angle = satelliteAngle(index, suggestions.length) * (Math.PI / 180);
            const x = centerX + satelliteRadius * Math.cos(angle) - HIT_SIZE / 2;
            const y = currentY - satelliteRadius * Math.sin(angle) - HIT_SIZE / 2;
            return (
              <Animated.View
